...
The goal was simple: use real-world data to cut through the noise and figure out what skills are actually worth learning and what jobs are worth chasing.
...
The dataset includes real-world data job postings from 2023 with information such as:
...
LEFT JOIN
    company_dim AS companies ON companies.company_id = jpf.company_id
WHERE
...
* **Wide Salary Range:** The top 10 paying data analyst roles span from $184,000 to $650,000, showing strong salary potential in the field.
...
* **Salary Insight:** The Mantys role ($650,000) is a major outlier — almost double the next highest salary (Meta at $336,500). It likely includes additional compensation or location-based adjustments. Without it, salaries fall into a tighter range between $184,000 and $336,500.

* **Health Insurance:** Only 4 out of the 10 roles include health insurance — AT&T, SmartAsset (both listings), and Get It Recruit. Interestingly, the two highest-paying roles don’t include it.
...
* Hover over each bar to see the exact salary and company

---

## 2. Skills Required for Top-Paying Jobs

To focus on the skills behind top-paying roles, I reused a smaller version of my first query using a CTE. In that section, I filtered for the top-paying remote data analyst jobs based on yearly salary.
...
* **Programming** is the largest category with 22 entries (32.8%). This mainly comes from SQL, Python, and R appearing across most roles.
...
* **ERM Data Analyst at Get It Recruit ($184,000)** requires the fewest skills — just SQL, Python, and R.

* **Principal Data Analyst roles at SmartAsset ($186,000 and $205,000)** both require 9 skills and share nearly the same skill mix.
...
### **Top 5 In-Demand Skills (All Job Postings)**
...
### **Top 5 Most In-Demand Skills for Data Analyst Roles**

![Top 5 Most In-Demand Skills for Data Analyst Roles](assets/chart_top5_demanded_skills.png)
...
* **Tableau — 790 listings (18.0%)**
  Tableau is the leading visualisation tool in this dataset, slightly ahead of Python.

* **Python — 734 listings (16.7%)**
  Python appears close behind Tableau. Its demand comes from automation, analysis, and data handling tasks.

* **R — 496 listings (11.3%)**
  R has the lowest demand among the five but still appears consistently in analyst roles.
...
| --- | ---------- | ------------ | ------------------ |
| 1   | go         | 19           | 121,599.32         |
| 2   | hadoop     | 17           | 113,989.03         |
| 3   | azure      | 19           | 113,149.71         |
| 4   | bigquery   | 11           | 112,772.73         |
| 5   | snowflake  | 28           | 112,616.64         |
| 6   | ssis       | 11           | 106,381.82         |
| 7   | looker     | 43           | 103,732.19         |
| 8   | aws        | 23           | 103,688.89         |
| 9   | redshift   | 12           | 103,498.58         |
| 10  | python     | 165          | 100,254.86         |
| 11  | r          | 105          | 99,900.63          |
| 12  | jira       | 12           | 99,754.08          |
| 13  | tableau    | 163          | 98,607.77          |
| 14  | ssrs       | 13           | 98,338.46          |
| 15  | nosql      | 12           | 98,198.21          |
| 16  | alteryx    | 11           | 97,601.36          |
| 17  | sql server | 27           | 97,514.81          |
| 18  | oracle     | 22           | 96,322.30          |
| 19  | javascript | 11           | 95,740.00          |
| 20  | sql        | 290          | 95,585.95          |
| 21  | power bi   | 68           | 95,368.61          |
| 22  | sas        | 42           | 94,716.35          |
| 23  | powerpoint | 42           | 89,515.80          |
| 24  | flow       | 18           | 88,125.00          |
...
*(Shows how often each skill appears, using the same colour groups as the salary chart.)*
...
*(Shows how far each skill is from matching its pay and demand rankings.)*
...
* **Data Aggregation**
  I worked heavily with `COUNT`, `AVG`, and `GROUP BY` to break down large datasets into meaningful insights. This helped me understand patterns in skill demand and salary trends.
...
1. **SQL and Excel are the baseline skills**
   They rank highly across both datasets (with and without health insurance), showing they are consistently expected in data analyst roles.

2. **High demand doesn’t always mean high pay**
   SQL has the highest demand but sits lower in average salary, while specialised tools like Go and Hadoop pay more despite appearing less often.

3. **Top-paying roles don’t always require more skills**
...
5. **The best path combines demand and salary**
...
Working on this project helped me understand the data job market more clearly.
